...
@MainActor
public final class MixedInboundAdapter: InboundNetworkAdapter {
    ...
    public func start() async throws {
        ...
        logger.info("Starting Mixed inbound adapter on \(configuration.listenAddress):\(configuration.port)")
        ...
        let bootstrap = ServerBootstrap(group: eventLoopGroup)
            ...
            .childChannelInitializer { (channel: any Channel) -> EventLoopFuture<Void> in
                let mixedInbound = MixedInbound(logger: self.logger)
                let promise: EventLoopPromise<Void> = channel.eventLoop.makePromise(of: Void.self)
                Task { @MainActor in
                    do {
                        try await mixedInbound.handle(context: channel.pipeline.context, connection: channel)
                        promise.succeed()
                    } catch {
                        promise.fail(error)
                    }
        ...
        serverChannel = try await bootstrap
            ...
            .get()
        isRunning = true
        logger.info("Mixed inbound adapter started")
    }

    public func stop() async throws {
        guard isRunning else { return }

        logger.info("Stopping Mixed inbound adapter")
        ...
        isRunning = false
        logger.info("Mixed inbound adapter stopped")
    }
    ...
    public func serve() async throws {
        ...
        logger.debug("Mixed inbound adapter is serving connections")
    }
}
